...
public:
	...
	OneServiceImpl(const char *hp,unsigned int port, bool forceQuic, bool verbose, bool noEncrypt, const char *publicIp):
		_homePath((hp) ? hp : ".")
		...
		,_forceQuic(forceQuic)
		,_verbose(verbose)
		,_noEncrypt(noEncrypt)
		,_publicIp((char *)publicIp)
		,_primaryPort(port)
		...
		,_ssoRedirectURL()
	{
		...
		_binder.forceQuic(forceQuic);
		_binder.disableEncrypt(_noEncrypt);
		_phy.setVerbose(_verbose);
		_phy.setPublicIp(_publicIp);
	...
	virtual ReasonForTermination run()
	{
		try {
...

			{
				...
				_node->disableEncrypt(_noEncrypt);
			}
			...
			_phy.setPrimaryPort(_primaryPort);
			...
			if (!_forceQuic && _allowSecondaryPort) {
				if (_secondaryPort) {
...
#ifdef ZT_USE_MINIUPNPC
			if (!_forceQuic && _portMappingEnabled) {
    ...
    void startHTTPControlPlane() {
		...
		_controlPlane.Get("/status", [&, setContent](const httplib::Request &req, httplib::Response &res) {
			...
			settings["forceQuic"] = OSUtils::jsonBool(settings["forceQuic"],_forceQuic);
	...
	void applyLocalConfig()
	{
		...
		//_forceQuic = OSUtils::jsonBool(settings["forceQuic"],false);
		//_binder.forceQuic(_forceQuic);
	...
	inline void phyOnDatagram(PhySocket *sock,void **uptr,const struct sockaddr *localAddr,const struct sockaddr *from,void *data,unsigned long len)
	{
		...
		if (_verbose)
		{
			...
			printf("Sock [%p] receiving UDP from %s:%d len %u\n", sock, Host, Port, len);
		}
	...
	inline void phyOnQuicConnect(PhySocket *sock,void **uptr,bool success) 
	{

	}

	inline void phyOnQuicAccept(PhySocket *sockL,PhySocket *sockN,void **uptrL,void **uptrN,const struct sockaddr *from)
	{

	}

	void phyOnQuickClose(PhySocket *sock,void **uptr)
	{

	}

	void phyOnQuicData(PhySocket *sock,void **uptr,void *data,unsigned long len)
	{

	}

	inline void phyOnQuicWritable(PhySocket *sock,void **uptr)
	{
		...
		if (!_forceQuic) {
			...
			if (_verbose)
			{
				...
				if (addr->ss_family == AF_INET6) {
					struct sockaddr_in6 *addrx = (struct  sockaddr_in6 *) addr;
					socklen_t Hlen = sizeof(Host);
					Port = ntohs(addrx->sin6_port);
					inet_ntop(AF_INET6, &addrx->sin6_addr.__in6_u, Host, Hlen);
				} else {
					struct sockaddr_in *addrx = (struct  sockaddr_in *) addr;
					socklen_t Hlen = sizeof(Host);
					Port = ntohs(addrx->sin_port);
					inet_ntop(AF_INET, &addrx->sin_addr.s_addr, Host, Hlen);
				}
				
				printf("Sock [%p] sending UDP to %s:%d len %u\n", (PhySocket *)((uintptr_t)localSocket), Host, Port, len);
			}
			...
			return _phy.quicSend(localSocket, (const struct sockaddr *)addr,data,len) ? 0 : -1;
		}
	}
...
OneService *OneService::newInstance(const char *hp,unsigned int port,bool f,bool v,bool n,const char *ip) { return new OneServiceImpl(hp,port,f,v,n,ip); }
