...
static void printHelp(const char *cn,FILE *out)
{
	...
	fprintf(out,"  -m                - Use msquic for transport" ZT_EOL_S);
	fprintf(out,"  -V                - Verbose message" ZT_EOL_S);
	fprintf(out,"  -n                - No Salsa Encryption" ZT_EOL_S);
	fprintf(out,"  -S<size>          - msquic send buffer size" ZT_EOL_S);
}

class _OneServiceRunner
{
public:
	_OneServiceRunner(const char *pn,const std::string &hd,unsigned int p, bool f, bool v, bool n, const char *ip) : progname(pn),returnValue(0),port(p),homeDir(hd),forceQuic(f),verbose(v),noEncrypt(n),publicIp((char *)ip) {}
	void threadMain()
		throw()
	{
		try {
			for(;;) {
				zt1Service = OneService::newInstance(homeDir.c_str(),port, forceQuic, verbose, noEncrypt, publicIp);
...
extern char *ZeroTier::sslKeyLogFile;
extern uint32_t ZeroTier::QUICSendBufferSize;
...
#endif
{
	...
	if (sslKeyLogFile && strlen(sslKeyLogFile) == 0)
		sslKeyLogFile = NULL;
	if (sslKeyLogFile)
		fprintf(stdout, "SSLKEYLOGFILE=%s\n", sslKeyLogFile);
		...
		if (argv[i][0] == '-') {
			switch(argv[i][1]) {
				...
				case 'V': // verbose message
					verbose = true;
					break;
				case 'n': // verbose message
					noEncrypt = true;
					break;
				case 'S':
					QUICSendBufferSize = atoi(argv[i] + 2);
					break;
	...
	_OneServiceRunner thr(argv[0],homeDir,port,forceQuic,verbose,noEncrypt,publicIp);
	thr.threadMain();
