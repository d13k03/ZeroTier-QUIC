...
#define ZT_BINDER_REFRESH_PERIOD 30000000
...
class Binder {
  private:
	struct _Binding {
		_Binding() : udpSock((PhySocket*)0), tcpListenSock((PhySocket*)0), quicListenSock((PhySocket *)0)
		{
		}
		PhySocket* udpSock;
		PhySocket* tcpListenSock;
		PhySocket* quicListenSock;
		InetAddress address;
		char ifname[256] = {};
	};

  public:
	Binder() : _bindingCount(0), _forceQuic(false), _noEncrypt(false)
	{
	}

	void forceQuic(bool force) { _forceQuic = force; }
	...
	template <typename PHY_HANDLER_TYPE> void closeAll(Phy<PHY_HANDLER_TYPE>& phy)
	{
		...
		for (unsigned int b = 0, c = _bindingCount; b < c; ++b) {
			if (!_forceQuic) {
				...
				//phy.close(_bindings[b].quicListenSock, false);
				phy.close(_bindings[b].udpSock, false);
			}
		...
		for (unsigned int b = 0; b < oldBindingCount; ++b) {
			if (localIfAddrs.find(_bindings[b].address) != localIfAddrs.end()) {
				...
				if (!_forceQuic) {
					...
					//PhySocket* const udps = _bindings[b].quicListenSock;
					PhySocket* const udps = _bindings[b].udpSock;
					_bindings[b].udpSock = (PhySocket*)0;
					phy.close(udps, false);
				}
		...
		for (std::map<InetAddress, std::string>::const_iterator ii(localIfAddrs.begin()); ii != localIfAddrs.end(); ++ii) {
			...
			if (bi == _bindingCount) {
				if (!_forceQuic) {
					...
					udps = phy.quicListen(reinterpret_cast<const struct sockaddr*>(&(ii->first)), (void*)0);
				}
				...
				if ((udps) && (tcps)) {
					...
					if (ii->second.length() > 0) {
						...
						int fd;
						if (!_forceQuic) {
							fd = (int)Phy<PHY_HANDLER_TYPE>::getDescriptor(udps);
							if (fd >= 0)
					...
					if (_bindingCount < ZT_BINDER_MAX_BINDINGS) {
						if (!_forceQuic) {
							...
							//_bindings[_bindingCount].quicListenSock = udps;
							_bindings[_bindingCount].udpSock = udps;
						}
	...
	bool getNoEncrypt() const { return _noEncrypt; }
	void disableEncrypt(bool b = true) { _noEncrypt = b; }
